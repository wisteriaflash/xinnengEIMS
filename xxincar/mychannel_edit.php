...
else if($dopost=="export")
{
	...
	$row = $dsql->GetOne("Select * From `#@__channeltype` where id='$id' ");
	$channelconfig = '';
	$row['maintable'] = ereg_replace($cfg_dbprefix,'#@__',$row['maintable']);
	$row['addtable'] = ereg_replace($cfg_dbprefix,'#@__',$row['addtable']);
	foreach($row as $k=>$v)
	{
		if($k=='fieldset') $v = "\r\n$v\r\n";
...
else if($dopost=="gettemplets")
{
	...
	$msg = "
		文档模板：{$defaulttemplate}/en/article_{$row['nid']}.htm
		<a href='tpl.php?acdir={$cfg_df_style}/en&action=edit&filename=article_{$row['nid']}.htm'>[修改]</a><br/>
		列表模板：{$defaulttemplate}/en/list_{$row['nid']}.htm
		<a href='tpl.php?acdir={$cfg_df_style}/en&action=edit&filename=list_{$row['nid']}.htm'>[修改]</a>
		<br/>
		频道封面模板：{$defaulttemplate}/en/index_{$row['nid']}.htm
		<a href='tpl.php?acdir={$cfg_df_style}/en&action=edit&filename=index_{$row['nid']}.htm'>[修改]</a>
	";
	$win->AddMsgItem("<div style='padding:20px;line-height:300%'>$msg</div>");
	$winform = $win->GetWindow("hand","");
	$win->Display();
