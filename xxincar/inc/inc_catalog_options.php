<?php
function GetOptionList($selid=0,$userCatalog=0,$channeltype=0,$showall=1)
{
	...
	if($selid > 0)
	{
		$row = $dsql->GetOne("Select id,typename,ispart,channeltype,reid From `#@__arctype` where id='$selid'");
		if($row['ispart']==1 && ($showall==1 || $row['reid']!=0) ) $OptionArrayList .= "<option value='".$row['id']."' class='option1' selected='1'>".$row['typename']."(封面频道)</option>\r\n";
		else if( ($showall==1 || $row['reid']!=0) ) $OptionArrayList .= "<option value='".$row['id']."' selected>".$row['typename']."</option>\r\n";
	}
  ...
  if($selid>0 && $showall==0)
  {
  	$query = "Select id,typename,ispart,channeltype From `#@__arctype` where ispart<2 And reid='$selid' order by sortrank asc ";
  }
	...
	while($row=$dsql->GetObject())
	{
		...
		//是否允许下级栏目模型不同
		else if($row->channeltype!=$channeltype && $channeltype!=0) $OptionArrayList .= ''; //$OptionArrayList .= "<option value='".$row->id."' class='option2'>".$row->typename."(".$channels[$row->channeltype].")</option>\r\n";
