...
else if($dopost=="viewArchives")
{
	...
	$query = "Select arc.*,ch.maintable,ch.addtable,ch.issystem,ch.editcon,
	          tp.typedir,tp.typename,tp.namerule,tp.namerule2,tp.ispart
           From `#@__arctiny` arc
           left join `#@__arctype` tp on tp.id=arc.typeid
	...
	if($trow['issystem'] != -1)
	{
		$arcQuery = "Select arc.*,tp.typedir,tp.typename,tp.namerule,tp.namerule2,tp.ispart
		           from `{$trow['maintable']}` arc left join `#@__arctype` tp on arc.typeid=tp.id
		           left join `#@__channeltype` ch on ch.id=arc.channel where arc.id='$aid' ";
		$arcRow = $dsql->GetOne($arcQuery);
		if($arcRow['ismake']==-1 || $arcRow['arcrank']!=0 || $arcRow['typeid']==0)
		{
	...
	$arcRow['namerule'],$arcRow['typedir'],$arcRow['filename']);
	$arcfile = GetFileUrl($arcRow['id'],$arcRow['typeid'],$arcRow['senddate'],$arcRow['title'],
	$arcRow['ismake'],$arcRow['arcrank'],$arcRow['namerule'],$arcRow['typedir'],$arcRow['filename']);
