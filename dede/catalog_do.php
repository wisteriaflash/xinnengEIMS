...
else if($dopost=="listArchives")
{
	...
	if($cid>0)
	{
		$row = $dsql->GetOne("Select tp.typename,ch.typename as channelname,ch.id,ch.mancon from `#@__arctype` tp left join `#@__channeltype` ch on ch.id=tp.channeltype where tp.id='$cid'");
		$gurl = $row["mancon"];
		$channelid = $row["id"];
...
else if($dopost=="upcatcache")
{
	...
	//导入普通模型微数据
	$sql = "insert into `#@__arctiny`(id, typeid, arcrank, channel, senddate, sortrank, mid)  
	        Select id, typeid, arcrank, channel, senddate, sortrank, mid from `#@__archives` ";
	$dsql->executenonequery($sql);
	...
	while($row = $dsql->GetArray())
	{
		...
		else
		{
			...
			$sql = "insert into `#@__arctiny`(id, typeid, arcrank, channel, senddate, sortrank, mid)  
			        Select aid, typeid, arcrank, channel, senddate, 0, mid from `$tb` ";
...
function GetSunList();
-----------*/
else if($dopost=="GetSunList")
{
	require_once(DEDEADMIN."/inc/inc_catalog_options.php");
	AjaxHead();
	$typeOptions = GetOptionList($tid, $cuserLogin->getUserChannel(), $cid, 0);
