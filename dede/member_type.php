...
if($dopost=="save")
{
	...
	for(;$startID<=$endID;$startID++)
	{
		...
		if(isset(${'check_'.$startID}))
		{
			if($pname!='')
			{
				$query = "update #@__member_type set pname='$pname',rank='$rank',exptime='$exptime' where aid='$aid'";
			}
		}
		else
		{
	...
	{
		$query = "Insert Into #@__member_type(rank,pname,exptime) Values('{$rank_new}','{$pname_new}','{$exptime_new}');";
		$dsql->ExecuteNoneQuery($query);
	}
