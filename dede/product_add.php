...
if($dopost!='save')
{
	...
	$channelid = $cInfos['id'];
	include DedeInclude("templets/product_add.htm");
	exit();
}
...
else if($dopost=='save')
{
	...
	{		
		...
		{			
			...
			if(strlen($v)<2 || !is_file($truefile))
			{
			 unset($_SESSION['bigfile_info'][$k]); continue;
			}
			$picinfo =  !empty(${'picinfook'.$k}) ? ${'picinfook'.$k} : '';			
			$bigpic .= "{dede:img text='$picinfo' width='120' height='120'} $v {/dede:img}\r\n";
			unset($_SESSION['bigfile_info'][$k]);
		}		
	}
	$bigpic = addslashes($bigpic);
	...
	//保存到主表
	$query = "INSERT INTO `#@__archives`(id,typeid,sortrank,flag,ismake,channel,arcrank,click,title,
    color,writer,source,litpic,pubdate,senddate,mid,description,keywords,filename)
    VALUES ('$arcID','$typeid','$sortrank','$flag','$ismake','$channelid','$arcrank','0','$title',
	...
	{
		$useip = GetIP();
		$query = "INSERT INTO `{$addtable}`(aid,typeid,redirecturl,bigpic,userip{$inadd_f}) Values('$arcID','$typeid','$redirecturl','$bigpic','$useip'{$inadd_v})";
		if(!$dsql->ExecuteNoneQuery($query))
		{
			$gerr = $dsql->GetError();
	...
	$msg = "
    　　请选择你的后续操作：
    <a href='product_add.php?cid=$typeid&channelid=6'><u>继续发布文档</u></a>
    &nbsp;&nbsp;
