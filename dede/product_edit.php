...
if($dopost!='save')
{
	...
	$bigpic = $addRow['bigpic'];
	$channelid = $arcRow['channel'];
	$tags = GetTags($aid);
	include DedeInclude("templets/product_edit.htm");
	exit();
}
...
else if($dopost=='save')
{
	...
	for($i=1;$i<=120;$i++)
	{
		...
		$bigpic .= "{dede:img text='$iinfo' width='120' height='120'} $iurl {/dede:img}\r\n";
		//有上传文件的情况
	...
	{
		$useip = GetIP();
		$iquery = "update `$addtable` set typeid='$typeid'{$inadd_f},redirecturl='$redirecturl',userip='$useip',bigpic='$bigpic' where aid='$id' ";
	...
	$msg = "
    　　请选择你的后续操作：
    <a href='product_add.php?cid=$typeid&channelid=6'><u>发布新文档</u></a>
    &nbsp;&nbsp;
	...
	$win->AddTitle("成功更改文档：");
	$win->AddMsgItem($msg);
	$winform = $win->GetWindow("hand","&nbsp;",false,"stat2");
	$win->Display();
}
