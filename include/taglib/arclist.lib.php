...
{
	global $dsql,$PubFields,$cfg_keyword_like,$cfg_index_cache,$_arclistEnv,$envs,$cfg_cache_type,$cfg_df_lang;
	$row = AttDef($row,10);
	...
	if(empty($typeid))
	{
		$row = $dsql->GetOne("Select id From `#@__arctype` where lang='$cfg_df_lang' And reid=0 ");
		if(is_array($row)) $typeid= $row['id'];
	}
	...
	$query = "Select arc.*,tp.typedir,tp.typename,tp.isdefault,tp.defaultname,tp.namerule,tp.namerule2,tp.ispart
		$addfieldsSql
  ...
  $artlist = '';
	...
	if(!empty($refObj->dtp->tplLang)) $dtp2->tplLang = $refObj->dtp->tplLang;
