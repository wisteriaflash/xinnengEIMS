...
global $PubFields,$pTypeArrays,$idArrary,$envs,$v1,$v2,$user_lang,$cfg_df_lang;
$pTypeArrays = array();
$idArrary = array();
$PubFields = array();
$envs = array();
...
$envs['idlist'] = '';
if(empty($user_lang)) {
	$user_lang = (isset($cfg_df_lang) ? $cfg_df_lang : 'gb2312');
}
...
/*************************************************
如果要获得文件的路径，直接用
GetFileUrl($aid,$typeid,$timetag,$title,$ismake,$rank,$namerule,$typedir)
即是不指定站点参数则返回相当对根目录的真实路径
**************************************************/
function GetFileUrl($aid,$typeid,$timetag,$title,$ismake=0,$rank=0,$namerule='',$typedir='', $filename='')
{
	$articleUrl = GetFileName($aid,$typeid,$timetag,$title,$ismake,$rank,$namerule,$typedir,$filename);
...
//获得新文件名(本函数会自动创建目录)
function GetFileNewName($aid,$typeid,$timetag,$title,$ismake=0,$rank=0,$namerule='',$typedir='',$filename='')
{
	$articlename = GetFileName($aid,$typeid,$timetag,$title,$ismake,$rank,$namerule,$typedir,$filename);
	if(ereg("\?",$articlename))
	{
		return $articlename;
	}
...
function GetFileName($aid,$typeid,$timetag,$title,$ismake=0,$rank=0,$namerule='',$typedir='',$filename='')
{
	if($rank!=0 || $ismake==-1 || $typeid==0)
	{
		//动态文章
...
//模板目录规则
function MfTemplet($tmpdir, $userlang='')
{
	global $userLang,$cfg_df_style,$cfg_templets_dir;
	if($userlang=='') $userlang = $userLang;
	$tmpdirn = $tmpdir = str_replace('{style}', $cfg_df_style, $tmpdir);
	$tmpdir = str_replace('{lang}', $userlang, $tmpdir);
	$tmpdir = ereg_replace('/{1,}', '/', $tmpdir);
	if(!@file_exists($cfg_templets_dir.'/'.$tmpdir))
	{
		$tmpdir = str_replace('{lang}', 'en', $tmpdirn);
		$tmpdir = ereg_replace('/{1,}', '/', $tmpdir);
	}
	return $tmpdir;
}
