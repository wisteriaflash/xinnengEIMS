...
class DedeVote
{
	...
	//获得项目的投票表单
	function GetVoteForm($lineheight=24,$tablewidth="100%",$titlebgcolor="#EDEDE2",$titlebackgroup="",$tablebg="#FFFFFF",$itembgcolor="#FFFFFF",$lang="")
	{
		global $userLang;
		$userLang = $lang;
		//省略参数
		...
		$items.="<tr align='center'><td height='$lineheight' bgcolor='$titlebgcolor' $titlebackgroup><strong>".$this->VoteInfos['votename']."</strong></td></tr>\r\n";
		//echo $this->VoteCount;
		if($this->VoteCount > 0)
		{
			...
			$items .= "<tr><td height='28' align='center'>\r\n";
			$items .= "<input type='submit' class='nbt' name='vbt1' value='".GetLang('votesend')."' /> &nbsp;\r\n";
			$items .= "<input type='button' class='nbt' name='vbt2' ";
			$items .= "value='".GetLang('viewrs')."' onClick=\"window.open('".$GLOBALS['cfg_phpurl']."/vote.php?dopost=view&aid=".$this->VoteID."');\" /></td></tr>\r\n";
		}
		$items.="</form>\r\n</table>\r\n";
		return $items;
	}
	...
	function GetVoteResult($tablewidth="600",$lineheight="24",$tablesplit="40%")
	{
		...
		$res = "<table width='$tablewidth' border='0' cellspacing='0' cellpadding='0' style='margin-bottom:20px'>\r\n";
		$res .= "<tr height='8'><td width='$tablesplit'></td><td></td></tr>\r\n";
		$i=1;
		foreach($this->VoteNotes as $k=>$arr)
		{
			$res .= "<tr height='$lineheight'><td style='border-bottom:1px dashed #dedede'>".$i."、".$arr['name']."</td>";
			$c = $arr['count'];
			$res .= "<td style='border-bottom:1px dashed #dedede'>
			<table border='0' cellspacing='0' cellpadding='2' width='".(($c/$totalcount)*100)."%'><tr><td height='16' background='img/votebg.gif'>".$arr['count']."</td></tr></table>
			</td></tr>\r\n";
			$i++;
		}
