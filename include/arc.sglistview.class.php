...
class SgListView
{
	...
	function GetArcList($limitstart=0,$row=10,$col=1,$titlelen=30,$listtype="all",$orderby="default",$innertext="",$tablewidth="100",$ismake=1,$orderWay='desc')
	{
		...
		if(ereg('hot|click',$orderby) || $this->sAddTable)
		{
			...
			tp.ispart,arc.aid,arc.aid as id,arc.typeid,
			$addField
		...
		for($i=0;$i<$row;$i++)
		{
			...
			for($j=0;$j<$col;$j++)
			{
				if($row = $this->dsql->GetArray("al"))
				{
					...
					$row['typeurl'] = GetTypeUrl($row['typeid'],MfTypedir($row['typedir']),$row['isdefault'],$row['defaultname'],$row['ispart'],$row['namerule2']);
