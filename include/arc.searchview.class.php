...
//Copyright 2004-2006 by DedeEIMS.com itprato
//本类的用途是用于文档搜索
@set_time_limit(0);
...
class SearchView
{
	...
	{
		global $cfg_search_max,$cfg_search_maxrc,$cfg_search_time,$lang;
		if(empty($upagesize))
		{
			$upagesize = 10;
		}
		...
		$this->CountRecord();
		$tempfile = DEDETEMPDIR."/".$GLOBALS['cfg_df_style']."/{$this->Lang}/search.htm";
		$tempfile2 = DEDETEMPDIR."/".$GLOBALS['cfg_df_style']."/en/search.htm";
	...
	function CountRecord()
	{
		...
		$cquery = "Select * From `#@__archives` arc left join `#@__arctype` tp on tp.id=arc.typeid where tp.lang='{$this->Lang}' And ".$this->AddSql;
		$hascode = md5($cquery);
		...
		else
		{
			if($this->TotalResult==-1)
			{
				...
				$this->dsql->executenonequery($insert);
	...
	$imgwidth=120,$imgheight=90,$achanneltype="all",$orderby="default",$innertext="",$tablewidth="100")
	{
		...
		//搜索
		$query = "Select arc.*,act.typedir,act.typename,act.isdefault,act.defaultname,act.namerule,act.namerule2,act.ispart
		from `#@__archives` arc left join `#@__arctype` act on arc.typeid=act.id
		where act.lang='{$this->Lang}' And {$this->AddSql} $ordersql limit $limitstart,$row";
		...
		for($i=0;$i<$row;$i++)
		{
			...
			for($j=0;$j<$col;$j++)
			{
				...
				if($row = $this->dsql->GetArray("al"))
				{
					...
					$row["ismake"],$row["arcrank"],$row["namerule"],$row["typedir"],$row['filename']);
					...
					$row["typeurl"] = GetTypeUrl($row["typeid"],$row["typedir"],$row["isdefault"],$row["defaultname"],$row["ispart"],$row["namerule2"]);
	...
	function GetPageListDM($list_len)
	{
		...
		{
			return GetLang('total').' 1 '.GetLang('page')."/".$this->TotalResult.' '.GetLang('records');
		}
		if($this->TotalResult == 0)
		{
			return GetLang('total').' 0 '.GetLang('page')."/".$this->TotalResult.' '.GetLang('records');
		}
		$purl = $this->GetCurUrl();
		...
		$infos = "<td>".GetLang('total')."<b>".$this->TotalResult."</b>".GetLang('records')."/Max: <b>{$totalpage}</b>".GetLang('page')." </td>\r\n";
		...
		if($this->PageNo != 1)
		{
			$prepage.="<td width='50'><a href='".$purl."PageNo=$prepagenum'>".GetLang('prepage')."</a></td>\r\n";
			$indexpage="<td width='30'><a href='".$purl."PageNo=1'>".GetLang('firstpage')."</a></td>\r\n";
		}
		else
		{
			$indexpage="<td width='30'>".GetLang('firstpage')."</td>\r\n";
		}
		if($this->PageNo!=$totalpage && $totalpage>1)
		{
			$nextpage.="<td width='50'><a href='".$purl."PageNo=$nextpagenum'>".GetLang('nextpage')."</a></td>\r\n";
			$endpage="<td width='30'><a href='".$purl."PageNo=$totalpage'>".GetLang('lastpage')."</a></td>\r\n";
		}
		else
		{
			$endpage="<td width='30'>".GetLang('lastpage')."</td>\r\n";
		}
