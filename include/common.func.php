...
//返回特定的语言资源串
function GetLang($rsid, $lang='')
{
	global $userLang;
	if($lang=='') $lang = $userLang;
	$lang = ereg_replace('[^0-9a-z]', '', $lang);
	
	if(!isset($GLOBALS['langArr_'.$lang]) && file_exists(DEDEINC."/lang/{$lang}.utf8.php"))
	{
		require_once(DEDEINC."/lang/{$lang}.utf8.php");
	}
	
	if(isset($GLOBALS['langArr_'.$lang][$rsid]))
	{
		return $GLOBALS['langArr_'.$lang][$rsid];
	}
	else
	{
		return $rsid;
...
function ShowMsg($msg,$gourl='',$onlymsg=0,$limittime=0)
{
	...
	else
	{
      ...
      function JumpUrl(){
		...
		$rmsg .= "<div style='padding:6px;font-size:12px;border-bottom:1px solid #D1DDAA;background:#DBEEBD url({$GLOBALS['cfg_phpurl']}/img/wbg.gif)';'><b>DedeEIMS Message!</b></div>\");\r\n";
		...
		if($onlymsg==0)
		{
			...
			{
				$rmsg .= "<br /><a href='{$gourl}'>".(GetLang('msg_browloading')=='msg_browloading' ? '正在转向指定页面，如果浏览器没反应请点击这里...' : GetLang('msg_browloading'))."</a>";
			}
			$rmsg .= "<br/></div>\");\r\n";
			if($gourl!="javascript:;" && $gourl!='')
			{
...
function GetOneArchive($aid)
{
	...
	if($chRow['issystem']!=-1)
	{
		$nquery = " Select arc.*,tp.typedir,tp.topid,tp.namerule From `{$chRow['maintable']}` arc left join `#@__arctype` tp on tp.id=arc.typeid
		            where arc.id='$aid' ";
	}
	else
	{
		$nquery = " Select arc.*,1 as ismake,'' as filename,tp.typedir,tp.topid,tp.namerule
	...
	$arcRow['typedir'],$arcRow['filename']);
	return $reArr;
