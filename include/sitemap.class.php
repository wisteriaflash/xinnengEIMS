...
class SiteMap
{
	...
	function GetSiteMap($maptype="site")
	{
		...
		if($maptype=="rss")
		{
			$this->dsql->SetQuery("Select id,typedir,isdefault,defaultname,typename,ispart,namerule2 From #@__arctype where ishidden<>1 And reid=0 And ispart<>2 order by sortrank");
		}
		else
		{
			$this->dsql->SetQuery("Select id,typedir,isdefault,defaultname,typename,ispart,namerule2 From #@__arctype where reid=0 And ishidden<>1 order by sortrank");
		}
		$this->dsql->Execute(0);
	...
	function LogicListAllSunType($id,$maptype)
	{
		...
		if($maptype=="rss")
		{
			$this->dsql->SetQuery("Select id,typedir,isdefault,defaultname,typename,ispart,namerule2 From #@__arctype where reid='".$id."' And ishidden<>1 And ispart<>2 order by sortrank");
		}
		else
		{
			$this->dsql->SetQuery("Select id,typedir,isdefault,defaultname,typename,ispart,namerule2 From #@__arctype where reid='".$id."' And ishidden<>1 order by sortrank");
		}
