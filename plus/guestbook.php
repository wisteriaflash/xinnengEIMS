...
else if($action=='save')
{
	...
	if($needCheck==1)
	{
		...
		$msg = "
		<b>成功发送你的留言！<br /><br />
		<a href='guestbook.php' style='font-size:14px;font-weight:bold'><u>点击此返回&gt;&gt;</u></a>";
  	$wintitle = "发表留言成功提示";
		$wecome_info = "成功发表留言：";
		$win = new OxWindow();
...
else
{
	...
	$dlist->pageSize = 10;
	$dlist->SetParameter('lang',$userLang);
	$dlist->SetParameter('gotopagerank',$gotopagerank);
	$dlist->SetTemplate(DEDETEMPDIR.'/plus/guestbook.htm');
	$dlist->SetSource($sql);
  $dlist->Display();
}
?>