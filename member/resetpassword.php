...
require_once(DEDEMEMBER."/inc_pwd_functions.php");
if(empty($dopost))
{
	$dopost = '';
}
...
elseif($dopost == 'getpwd')
{
	...
	if(!is_array($member))
	{
		ShowMsg('对不起，用户ID或邮箱错误！', '-1');
		exit();
	}

	//以邮件方式取回密码；判断系统邮件服务是否开启
...
elseif($dopost == 'getpasswd')
{
	...
	$row = $db->GetOne("Select * From `#@__member_pwd_tmp` where mid = '$mid'");
	if(empty($row))
	{
		ShowMsg('对不起，请不要非法提交', 'login.php');
		exit();
	...
	if(empty($setp))
	{
		...
		{
			$db->executenonequery("DELETE FROM `#@__member_pwd_tmp` WHERE `md` = '$id'; ");
	...
	elseif($setp == 2)
	{
		...
		if($row['pwd'] == $sn)
		{
			if($pwd != '')
			{
				...
				{
					$pwdok = md5($pwdok);
					$sql = "DELETE FROM `#@__member_pwd_tmp` WHERE `mid` = '$id'; ";
