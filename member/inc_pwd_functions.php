...
function newmail($mid,$userid,$mailto,$type,$send)
{
	...
	{
		$key = md5($randval);
		$sql = "INSERT INTO `#@__member_pwd_tmp` (`mid` ,`membername` ,`pwd` ,`mailtime`)VALUES ('$mid', '$userid',  '$key', '$mailtime');";
	...
	{
		$key = md5($randval);
		$sql = "UPDATE `#@__member_pwd_tmp` SET `pwd` = '$key',mailtime = '$mailtime'  WHERE `mid` ='$mid';";
		if($db->ExecuteNoneQuery($sql))
		{
			if($send == 'Y')
			{
...
{
	global $db;
	$sql = "Select mid,email From `#@__member` where email='$mail' AND userid = '$userid' ";
	$row = $db->GetOne($sql);
	if(!is_array($row))
	{
		return 0;
...
function sn($mid,$userid,$mailto, $send = 'Y')
{
	...
	$tptim= (60*10);
	$dtime = time();
	$sql = "Select * From `#@__member_pwd_tmp` where mid = '$mid'";
	$row = $db->GetOne($sql);
	if(!is_array($row))
	{
